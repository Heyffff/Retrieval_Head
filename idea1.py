...
def plot_attention_scores(attention_matrix, input_tokens, generated_sentences, model):
    ...
    for i in range(num_rows):
        row_data = attention_matrix[i, :]
        plt.figure(figsize=(15, 6))
        # 绘制线条，设置线条颜色和样式
        plt.plot(row_data, color='blue', linewidth=2, linestyle='-', marker='o', markersize=5)
        print(generated_sentences[i] + f": {normalize(row_data)[0]}")
        # 设置 x 轴刻度和标签
        tick_positions = np.arange(num_cols)
        plt.xticks(tick_positions, input_tokens, rotation=90, ha='left', fontsize=4)
        # 添加网格线
        plt.grid(True, linestyle='--', alpha=0.7)
        # 设置坐标轴标签和标题的字体大小和颜色
        plt.xlabel('Input Tokens', fontsize=16, color='black')
        plt.ylabel('Attention Score', fontsize=16, color='black')
        plt.title(f'Attention Distribution for Subsentence: {generated_sentences[i]}', fontsize=18, color='black')
        # 设置坐标轴范围
        plt.ylim(0, max(row_data) * 1.1)
        # 优化布局
        plt.tight_layout()
        # 保存图片
        plt.savefig(f"./viz/attention_score/case1/{model}_row_{i}.png")
        plt.close()
...
if __name__ == "__main__":
    ...
    # 读取模型
    if "home" in args.model:
        #"/home/pkuiti26/xiaobei/llmbase/qwen/Qwen2___5-32B-Instruct"
        model_path = f"{args.model}"
        args.model = str(args.model).split("/")[-1]
    else:
    ...
    for r in output_ranges:
        ...
        sentence_attention_final = torch.cat((sentence_attention_final, torch.mean(decode_attention[r[0]:r[1], :], dim=0, keepdim=True)), dim=0)

    # 绘制热力图
    if args.sentence:
        plot_attention_scores(sentence_attention_final.detach().numpy(), input_tokens, output_sentences, args.model)
    else:
