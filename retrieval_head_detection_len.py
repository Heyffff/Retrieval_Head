...
class LLMNeedleHaystackTester:
    ...
    def __init__(self,
                ...
                haystack_dir="./haystack_for_detect_len",
        ...
        self.haystack_dir_list = [f"{haystack_dir}/part{i}" for i in range(1, 216)]
    ...
    def run_test(self, args):
        ...
        if args.max:
            ...
            self.document_depth_percents = self.document_depth_percents[:1]
        else:
            self.context_lengths = self.context_lengths[:1]
            self.document_depth_percents = self.document_depth_percents[-1:]
    ...
    def evaluate_and_log(self, context_length, depth_percent):
        ...
        if self.needle_start == -1 and self.needle_end == -1:
            return
        with torch.no_grad():
        ...
        if self.save_contexts:
            ...
            if not os.path.exists(f'contexts/{self.model_version}{"-len-max" if args.max else "-len-min"}'):
                os.makedirs(f'contexts/{self.model_version}{"-len-max" if args.max else "-len-min"}')

            with open(f'contexts/{self.model_version}{"-len-max" if args.max else "-len-min"}/{context_file_location}_context.txt', 'w') as f:
                f.write(context)
            
        if self.save_results:
            # Save the context to file for retesting
            if not os.path.exists(f'results/graph/{self.model_version}{"-len-max" if args.max else "-len-min"}'):
                os.makedirs(f'results/graph/{self.model_version}{"-len-max" if args.max else "-len-min"}')
            
            # Save the result to file for retesting
            p = f'results/graph/{self.model_version}{"-len-max" if args.max else "-len-min"}/{context_file_location}_results.json'
    ...
    def start_test(self, args):
        ...
        if os.path.exists(f'head_score/{self.model_version}{"-len-max" if args.max else "-len-min"}.json'):
            with open(f'./head_score/{self.model_version}{"-len-max" if args.max else "-len-min"}.json', "r") as file:
                head_counter = json.loads(file.readline())
            for k,v in head_counter.items():
                self.head_counter[k] += v
        with open(f'head_score/{self.model_version}{"-len-max" if args.max else "-len-min"}.json', 'w') as f:
