...
class LLMNeedleHaystackTester:
    ...
    def __init__(self,
                ...
                haystack_dir="./haystack_for_detect_zh",
    ...
    def evaluate_and_log(self, context_length, depth_percent):
        ...
        question = f"基于上述提供的内容，问题： {self.retrieval_question}\n答案："
        '''
        ...
        score = Rouge().get_scores(preprocess_text(response), preprocess_text(self.real_needle))[0]['rouge-1']['r']*100
        ...
        if self.save_contexts:
            ...
            if not os.path.exists(f'contexts/{self.model_version}-zh'):
                os.makedirs(f'contexts/{self.model_version}-zh')

            with open(f'contexts/{self.model_version}-zh/{context_file_location}_context.txt', 'w') as f:
                f.write(context)
            
        if self.save_results:
            # Save the context to file for retesting
            if not os.path.exists(f'results/graph/{self.model_version}-zh'):
                os.makedirs(f'results/graph/{self.model_version}-zh')
            
            # Save the result to file for retesting
            p = f'results/graph/{self.model_version}-zh/{context_file_location}_results.json'
    ...
    def start_test(self, args):
        ...
        if os.path.exists(f"head_score/{self.model_version}-zh.json"):
            with open(f"./head_score/{self.model_version}-zh.json", "r") as file:
                head_counter = json.loads(file.readline())
            for k,v in head_counter.items():
                self.head_counter[k] += v
        with open(f"head_score/{self.model_version}-zh.json", 'w') as f:
